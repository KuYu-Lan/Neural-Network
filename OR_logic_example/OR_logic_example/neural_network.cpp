...
double** neural_network::nn_ervery_layer_output(double* input_value) {
	...
	for (int i = 1; i < layer_num; i++) {
		double *temp = NULL;
		temp = Fully_Connect_layer(input_value, nn_weight + weight_offset, nn_bias + bias_offset, i); //Every Single Layer Output
...
double* neural_network::nn_output(double* input_value) {
	...
	for (int i = 1; i < layer_num; i++) {
		delete []output;
		output = Fully_Connect_layer(input_value, nn_weight + weight_offset, nn_bias + bias_offset, i); //Every Single Layer Output
		weight_offset += layer_neurons_num[i - 1] * layer_neurons_num[i];
		bias_offset += layer_neurons_num[i];
	}
	return output;
